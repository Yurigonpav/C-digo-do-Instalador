...
# ── Gerenciador de configurações e diálogo avançado ─────────────────────────
try:
...
class JanelaPrincipal(QMainWindow):
    ...
    def _criar_menu(self):
        ...
        a_config = QAction("Configurações...", self)
        a_config.setShortcut("Ctrl+,")
        a_config.setToolTip("Abre as configurações avançadas do NetLab (limite de hosts, filtros, etc.)")
        a_config.triggered.connect(self._abrir_configuracoes)
        m_mon.addAction(a_config)
    ...
    def _criar_barra_status(self):
        ...
        self.lbl_hosts_info.setStyleSheet("color: #7f8c8d; font-size: 10px; padding: 0 6px;")
    ...
    def _abrir_configuracoes(self):
        ...
        if not CONFIG_MANAGER_DISPONIVEL:
            QMessageBox.warning(
                self, "Configurações indisponíveis",
                "O módulo de configurações não pôde ser carregado.\n"
                "Verifique os arquivos utils/config_manager.py e interface/dialog_configuracoes.py"
            )
            return

        dlg = DialogConfiguracoes(self._config_manager, parent=self)
    ...
    def _aplicar_configuracoes(self, config: dict):
        ...
        # 1. Limite de hosts
        novo_limite = config.get("limite_hosts", cfg.limite_hosts)
        ...
        novo_timer_s = config.get("timer_redescoberta_s", 30)
        ...
        for ip in cfg.hosts_excluidos:
            if ip in getattr(self.painel_topologia, "visualizador", object()).dispositivos if hasattr(self.painel_topologia, "visualizador") else {}:
                try:
        ...
        # 5. Atualiza indicador de status
        self._atualizar_status_hosts()
    ...
    def _exibir_manual(self):
        ...
        CONTEUDO = {
            ...
            # ── REQUISITOS ────────────────────────────────────────────────────
            "req": CSS + """<body>
            <h2>Requisitos do Sistema (NetLab V5.0)</h2>
            <p class="muted">O NetLab agora conta com um instalador profissional que automatiza toda a configuração inicial.</p>
            <hr>

            <div class="box-ok">
              <b class="ok">Instalação Automatizada (Recomendado para Alunos)</b><br>
              O novo instalador executável (<code>NetLab_Educacional_Setup.exe</code>) cuida de tudo:<br>
              <ul>
                <li>Instala e configura o driver de captura <b>Npcap 1.82</b> automaticamente.</li>
                <li>Ativa o modo obrigatório <b>"WinPcap API-compatible Mode"</b> sem intervenção manual.</li>
                <li>Cria atalhos na Área de Trabalho que já solicitam privilégios de Administrador por padrão.</li>
                <li>Configura exceções automáticas no <b>Firewall do Windows</b> para o Servidor de Laboratório (porta 8080).</li>
              </ul>
              <i>Não é necessário instalar Python, venv ou digitar comandos no terminal! Basta avançar no assistente do instalador.</i>
            </div>

            <h3>Requisitos de Execução (Do Computador)</h3>
            <table>
              <tr><th>Componente</th><th>Requisito</th><th>Papel no NetLab</th></tr>
              <tr><td><b>Sistema Operacional</b></td><td>Windows 10 (Build 17763+) ou Windows 11 (64-bit)</td><td>Compatibilidade com PyQt6 e drivers modernos de rede.</td></tr>
              <tr><td><b>Npcap</b></td><td>Versão 1.82 (instalado junto com o Setup)</td><td>Driver de captura de pacotes de baixo nível em modo promíscuo.</td></tr>
              <tr><td><b>Privilégios</b></td><td>Administrador do Sistema</td><td>Obrigatório para que o driver interaja com a placa de rede física.</td></tr>
              <tr><td><b>Placa de Rede</b></td><td>Ethernet física ou Wi-Fi ativa</td><td>Interface de onde os pacotes de rede serão farejados e analisados.</td></tr>
            </table>

            <h3>Para Desenvolvedores (Execução via Código Fonte)</h3>
            <p>Caso deseje rodar a plataforma diretamente do código ou modificá-lo, o ambiente requer:</p>
            <table>
              <tr><th>Tecnologia</th><th>Versão</th><th>Finalidade</th></tr>
              <tr><td><b>Python</b></td><td>3.11 a 3.14 (64-bit)</td><td>Linguagem base do motor e da interface.</td></tr>
              <tr><td><b>PyQt6</b></td><td>6.4+</td><td>Biblioteca de interface gráfica e estilização escura.</td></tr>
              <tr><td><b>Scapy</b></td><td>2.7+</td><td>Motor de captura e manipulação detalhada de pacotes.</td></tr>
              <tr><td><b>pyqtgraph</b></td><td>0.13+</td><td>Plotagem em alta velocidade do gráfico de banda.</td></tr>
              <tr><td><b>Inno Setup</b></td><td>6.2+</td><td>Compilador para gerar o instalador empacotado.</td></tr>
              <tr><td><b>PyInstaller</b></td><td>6.x</td><td>Empacotador do executável standalone do projeto.</td></tr>
            </table>

            <div class="box">
              <b>Passos para Execução via Código Fonte:</b><br>
              <code>1.</code> Clone ou extraia os arquivos do projeto.<br>
              <code>2.</code> Crie o ambiente virtual: <code>python -m venv venv</code><br>
              <code>3.</code> Ative-o: <code>.\\venv\\Scripts\\Activate.ps1</code><br>
              <code>4.</code> Instale os pacotes: <code>pip install -r requirements.txt</code><br>
              <code>5.</code> Execute o programa: <code>python main.py</code>
            </div>
            </body>""",
            ...
            <div class="box-ok">
              <b class="ok">[DICA] Dica</b> — Abra o navegador e acesse qualquer site.
            ...
            <div class="box-warn">
              <b class="warn">[ATENÇÃO] Wi-Fi no Windows</b><br>
            ...
            <h3>Glossário Técnico Interativo</h3>
            <p>Os textos pedagógicos da aba Modo Análise integram um <b>Glossário Dinâmico</b>. Todos os termos técnicos fundamentais (como <code>HTTP</code>, <code>DNS</code>, <code>TTL</code>, <code>XSS</code>, <code>SYN</code>, etc.) aparecem sublinhados por uma linha pontilhada discreta.</p>
            <ul>
              <li><b>Acesso rápido:</b> Clique sobre qualquer termo destacado para abrir uma janela flutuante contextual.</li>
              <li><b>Conteúdo didático:</b> A janela exibe uma explicação direta do termo, o seu objetivo prático na rede, exemplos de aplicação técnica real e possíveis alertas de segurança correlatos.</li>
              <li><b>Sem obstrução:</b> O painel do glossário é livre de barras de título do sistema operacional e é 100% móvel. <b>Arraste-o com o mouse</b> para qualquer canto da tela para não atrapalhar seu campo de visão.</li>
              <li><b>Fechamento automático:</b> Clique fora do popup, no botão de fechar ou pressione a tecla <i>Esc</i> para fechá-lo instantaneamente e continuar a aula.</li>
            </ul>
            </body>""",
            ...
            <ol style="margin:6px 0 10px 0; padding-left:18px;">
              ...
              <li>Ajuste a interface (<b>Bind</b>) e a <b>Porta</b> de escuta</li>
              <li>Clique em <b>Iniciar Servidor</b></li>
              <li>O endereço de acesso aparece no painel:
                  <code>http://&lt;ip-selecionado&gt;:&lt;porta&gt;/</code></li>
            </ol>
            <p>Acesse pelo navegador de qualquer dispositivo na mesma rede local.
            Todos os dados do banco SQLite simulado ficam em memória RAM e são destruídos ao parar o servidor.</p>

            <h3>Configurações Avançadas de Rede</h3>
            <ul>
              <li><b>Seleção de Bind:</b> Selecione <code>127.0.0.1</code> para testes isolados neste computador ou <code>0.0.0.0</code> (ou o IP da sua placa Wi-Fi/Ethernet) para escutar na rede inteira e permitir conexões externas.</li>
              <li><b>Regra de Firewall Automática:</b> Quando marcada a opção <i>"Criar regra no Firewall do Windows"</i>, o NetLab executa comandos netsh para abrir a porta TCP de entrada selecionada no Firewall do Windows Defender e a fecha automaticamente ao parar o servidor. Isso remove qualquer barreira de tráfego em rede no laboratório físico de forma transparente!</li>
              <li><b>QR Code Dinâmico de Conexão:</b> Clique em <b>Exibir QR Code</b> para abrir um popup contendo a imagem do código QR do endereço LAN ativo. Os alunos podem simplesmente apontar a câmera do celular para se conectar instantaneamente ao site, eliminando a digitação propensa a erros de URLs complexas (IPs e portas).</li>
              <li><b>Diagnóstico de Conectividade HTTP:</b> Caso haja problemas de comunicação, clique em <i>"Reexecutar diagnóstico"</i>. O NetLab dispara requisições HTTP reais internas para sua rota local <code>/health</code> para validar o status da porta e mapear se há isolamento de clientes (guest isolation) na rede Wi-Fi local, fornecendo soluções claras para ambientes escolares.</li>
            </ul>

            <h3>Métricas em Tempo Real e Carga</h3>
            <p>O painel esquerdo apresenta quatro painéis dinâmicos de alta performance para monitoramento:</p>
            <ul>
              <li><b>TOTAL REQS:</b> Contagem total acumulada de requisições recebidas.</li>
              <li><b>REQS/SEG:</b> Frequência de requisições instantâneas recebidas, ideal para demonstrar ataques de força bruta ou negação de serviço.</li>
              <li><b>DADOS:</b> Volume total de tráfego de rede trocado (formatado dinamicamente em B, KB ou MB).</li>
              <li><b>CLIENTES:</b> Número de endereços IP de alunos únicos que interagiram com o servidor de laboratório.</li>
              <li><b>Carga:</b> Barra gráfica horizontal indicando o nível de carga e requisições/segundo processadas.</li>
            </ul>
            ...
            <table>
              ...
              <tr><td><code>/</code></td><td>Página inicial — estado da sessão (cookies visíveis)</td></tr>
              <tr><td><code>/login</code></td>
                  <td>SQL Injection (concatenação direta de query) + força bruta sem limite de tentativas</td></tr>
              <tr><td><code>/register</code></td>
                  <td>SQL Injection no comando INSERT + senhas gravadas em texto puro em banco de dados</td></tr>
              <tr><td><code>/alterar_senha</code></td>
                  <td>CSRF (Cross-Site Request Forgery) — permite a troca de senha do usuário logado via requisição sem token anti-CSRF</td></tr>
              <tr><td><code>/produtos?id=</code></td>
                  <td>SQL Injection no parâmetro <code>id</code> da URL (UNION SELECT permite roubar dados da tabela users)</td></tr>
              <tr><td><code>/busca?q=</code></td>
                  <td>XSS Refletido (Cross-Site Scripting) — parâmetro <code>q</code> refletido diretamente na página sem escape HTML</td></tr>
              <tr><td><code>/perfil?nome=</code></td>
                  <td>XSS Refletido — parâmetro <code>nome</code> impresso na interface de forma insegura</td></tr>
              <tr><td><code>/comentarios</code></td>
                  <td>XSS Armazenado + CSRF (comentários salvos no banco contêm tags html executáveis a todos os visitantes)</td></tr>
              <tr><td><code>/pedidos?id=</code></td>
                  <td>IDOR (Insecure Direct Object Reference) — alterando o id na URL acessa-se pedidos de qualquer usuário sem autenticação</td></tr>
              <tr><td><code>/usuarios</code></td>
                  <td>Divulgação de Informações Sensíveis — página aberta exibindo todos os usuários e senhas gravadas</td></tr>
              <tr><td><code>/api/usuarios</code></td>
                  <td>API JSON desprotegida expondo o dump completo do banco de dados em formato JSON</td></tr>
            </table>
            ...
            <ul>
              <li><b>Tabela de requisições:</b> hora, IP, método, endpoint, tamanho, tempo e payload completo da requisição.</li>
              <li><b>Log de alertas:</b> cada ataque detectado e alertado em tempo real por assinaturas SQLi, XSS, CSRF ou IDOR.</li>
              <li><b>Métricas:</b> total de requisições, dados, clientes únicos e carga.</li>
            </ul>

            <div class="box-warn">
              <b class="warn">[ATENÇÃO] Aviso de Segurança Importante</b><br>
              Este servidor implementa vulnerabilidades reais para fins didáticos. Use apenas em redes
              controladas (sala de aula, laboratório local ou host-only). Nunca exponha na internet ou redes públicas.
            </div>
            </body>""",
